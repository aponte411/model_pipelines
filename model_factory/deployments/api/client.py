...
@click.option('-s/-i',
              '--secure/--insecure',
              default=False,
              help='Use a secure channel or not',
              show_default=True)
@click.pass_context
def cli(ctx: Any, host: str, port: int, secure: bool):
    ...
    channel_context = (grpc.insecure_channel(server_addr)
                       if not secure else grpc.secure_channel(
                           server_addr, grpc.ssl_channel_credentials()))
...
@click.option('-cmp',
              ...
              type=str,
              required=False,
              help='Name of current competition')
@click.pass_context
def train(ctx: Dict, config: str, competition: str):
    ...
    with ctx.obj['channel_context'] as channel:
        ...
        train_request = numerai_pb2.TrainRequest(config=config,
...
@click.option('-sub',
              ...
              type=bool,
              required=False,
              help='Submit predictions to live NumerAI tournamenet')
@click.pass_context
def predict(ctx: Dict, config: str, competition: str, submit: bool):
    ...
    with ctx.obj['channel_context'] as channel:
        ...
        inference_request = numerai_pb2.InferenceRequest(
            config=config, competition=competition, submit=submit)
